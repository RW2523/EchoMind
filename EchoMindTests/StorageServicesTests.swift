...
@Suite struct StorageServicesTests {
    ...
    @Test func usageCountsRealBytes() async throws {
        ...
        let service = DefaultStorageUsageService(sessions: sessions, documents: documents, chunks: chunks)
        ...
        #expect(usage.totalBytes == usage.sessionsBytes + usage.documentsBytes + usage.indexBytes)
    }
    ...
    @Test func wipeRemovesEverything() async throws {
        ...
        let wipe = DefaultDataWipeService(sessions: sessions, documents: documents, chunks: chunks, chat: chat)
